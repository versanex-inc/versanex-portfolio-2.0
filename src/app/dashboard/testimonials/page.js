...
const AllReviewsDashboard = () => {
    ...
    async function fetchReviewsData() {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_HOST}/api/getReviews`
        );
        const reviewsData = await response.json();

        if (isMounted) {
          const reviewsArray = Array.isArray(reviewsData.result)
            ? reviewsData.result
            : [];
          ...
          reviewsArray.forEach((review) => {
            const rating =
              review.stars && review.stars.length > 0
  ...
  const handleSortChange = (sortOption) => {
    ...
    if (sortOption === "last-week") {
      ...
      filtered = filtered.filter(
        (review) => new Date(review.createdAt) >= lastWeek
      );
    } else if (sortOption === "last-month") {
      const lastMonth = new Date(now.setMonth(now.getMonth() - 1));
      filtered = filtered.filter(
        (review) => new Date(review.createdAt) >= lastMonth
      );
    } else if (sortOption === "last-year") {
      const lastYear = new Date(now.setFullYear(now.getFullYear() - 1));
      filtered = filtered.filter(
        (review) => new Date(review.createdAt) >= lastYear
      );
    }
    ...
    if (rating === "all-ratings") {
      ...
      const filtered = reviews.filter(
        (review) =>
          ...
          review.stars[0].rating === parseInt(rating)
      );
      setFilteredReviews(filtered);
    }
  };
  ...
  const handleLike = async (reviewId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/addReviewLike`,
        {
          method: "POST",
          headers: {
  ...
  const handleAddReview = async (newReview) => {
    try {
      ...
      if (data.success) {
        ...
        const rating =
          data.review.stars && data.review.stars.length > 0
            ? data.review.stars[0].rating
            : 0;
  ...
  return (
    <div className="main-reviews main-review-db">
      <FilterReview
        reviews={reviews}
      ...
      />
      <div className="reviews_about_page_gid">
        {filteredReviews.map((review) => {
          const user = getUserById(review.user);
          const currentUser = session
            ? users.find((user) => user.email === session.user.email)
            : null;
          const userLikesReview =
            currentUser &&
            review.likes.some((like) => like.userId === currentUser._id);
          ...
          return (
            <div className="review_one" key={review._id}>
              ...
              <div className="reviewed_user_name">
                <p className="reviewd_user_name_testi">{user ? user.name : "Unknown User"}</p>
                <span className='reviewd_user_undername'>
                <p>
                  {user ? user.reviews.length : 0} <span>Reviews</span>
                </p>
                -
                <p>{getTimeAgo(review.createdAt)}</p>
                </span>
              </div>
              ...
              <div className="review_user_main_review_content">
                <p className="review_user_content">{review.details}</p>
              </div>
              <div
                className={`review_reply_input_main ${
                  feedbackOpenStates[review._id] ? "review__input" : ""
                }`}
              >
                <div className="review_reply_content">
                  {review.feedback.map((fb) => (
                    <div className="review_reply_response" key={fb._id}>
                      <b>Response from VersaNex</b>
                      <p className='review_feedback_v'>{fb.feedback}</p>
                    </div>
                  ))}
                </div>
              ...
              <div className="like_review_reply">
                <div
                  className="like_review"
                  onClick={() => handleLike(review._id)}
                >
                  <FaThumbsUp className={userLikesReview ? "liked" : ""} />{" "}
                </div>
                <p className='review_like_count'>Likes : {likeCount}</p>
                <div
                  className="review_reply"
                  onClick={() => toggleFeedback(review._id)}
                >
                  Feedback
                </div>
              </div>
